...
// if we would go out of bounds, returns undefined
export const peekNeighor = (tileIndex: number, columns: number, rows: number, xDistance = 0, yDistance = 0) => {
  if (tileIndex + xDistance < 0 || tileIndex % columns + xDistance > columns) {
    return undefined
  }
  if (tileIndex + yDistance < 0 || tileIndex % rows + yDistance > rows) {
    return undefined
  }
